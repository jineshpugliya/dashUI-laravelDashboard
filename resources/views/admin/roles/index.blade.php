                                        ...
                                        <tr>
                                            ...
                                            <td class="align-content-center">
                                                @foreach ($role->permissions as $permission)
                                                    <span style="border-radius: 1em"
                                                        class="justify-center px-2 py-1 mr-2 text-xs text-bold text-white bg-gray-500">{{ $permission->name }}</span>
                                                @endforeach
                                            </td>
                                            <td class="text-center h-auto">
                                                @if (auth()->user()->hasRole($role->name) != $role->name)
                                                    <form id="roleDelete" action="{{ route('admin.roles.destroy', $role->id) }}"
                                                        ...
                                                        @can('Role delete')
                                                            <button class="cursor-pointer fas fa-trash text-secondary"
                                                                ...
                                                                data-bs-original-title="Delete Role"></button>
                                                        @endcan
                                                    </form>
                                                @endif
                                            </td>
                        ...
                        <form method="POST" action="{{ route('admin.roles.store') }}" role="form text-left">
                            ...
                            <div class="form-group">
                                ...
                                <div class="grid grid-cols-3 gap-4">
                                    @foreach ($permissions as $permission)
                                        <ul style="display: block; float: left; width: 25%; list-style: none">
                                            <li>
                                                <input type="checkbox" class="form-checkbox h-5 w-5 text-blue-600"
                                                    name="permissions[]" value="{{ $permission->id }}"><span
                                                    class="text-gray-700">{{ $permission->name }}</span>
                                            </li>
                                        </ul>
                                    @endforeach
                                </div>
                            ...
                            <div class="row">
                                <div class="d-flex justify-content-end">
                                    <button type="submit"
                                        class="btn bg-gradient-info btn-md mt-4 mb-4">{{ 'Add Role' }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
